...
// Do level order traversal, and in each level, put the first node of that level into the result
void leftView(Node *root, vi &res)
{
	if (!root)	return;
	...
	{
		int size = q.size();
		// We want the first node of each level to be pushed into the res vector
